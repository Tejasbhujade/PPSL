...
// Write the structure to the binary file outFile.write(reinterpret_cast<const char*>(&employee), sizeof(employee));
if (!outFile) {
    cerr << "Error: Could not open the file for writing!" << endl; 
    return;
}
...
if (inFile) {
   ...
   cerr << "Error: Could not read data from the binary filel" << endl;
}
